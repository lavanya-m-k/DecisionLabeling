...
class State:
    ...
    def find_videos(self):
        ...
        #     pass
        # return dirs #next(os.walk(DATA_DIR))[1]
    ...
    def update_file_names(self):
        if self.current_video:
            self.file_names = sorted(glob.glob(os.path.join(DATA_DIR, self.current_video, '*.jpg')),
    ...
    def set_side(self, side):
        if self.side != side:
            self.side = side
        self.notify_listeners("on_side_change")
