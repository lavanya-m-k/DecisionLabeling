...
from decisionlabeling.models import StateListener, SSHCredentials
from decisionlabeling.config import OUTPUT_DIR, SERVER_DIR
...
class SSHLogin(QGroupBox, StateListener):
    ...
    def on_connect_button_clicked(self):
        ...
        try:
            ...
            try:
                sftp.stat('decisionlabeling_server')  # Files already exist on the server
    ...
    def start_tracking_server(self):
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source siamMask/env/bin/activate && tmux new -d -s tracking "CUDA_VISIBLE_DEVICES=0 python -m tracker"')
    ...
    def start_tracking_servers(self):
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source siamMask/env/bin/activate && tmux new -d -s tracking_1 '
                                                             '"CUDA_VISIBLE_DEVICES=0 python -m tracker -p 8787"')

        stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source siamMask/env/bin/activate && tmux new -d -s tracking_2 '
                                                             '"CUDA_VISIBLE_DEVICES=0 python -m tracker -p 8788"')

        # stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source siamMask/env/bin/activate && tmux new -d -s tracking_3 '
    ...
    def start_detection_server(self):
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source detection/env/bin/activate && tmux new -d -s detection "CUDA_VISIBLE_DEVICES=0 python -m detector"')
    ...
    def start_detached_detection(self, seq_path, crop_area=None, detector="YOLO"):
        ...
        stdin, stdout, stderr = self.ssh_client.exec_command('cd decisionlabeling_server && source detection/env/bin/activate && tmux new -d -s detached '
